...
function TreeItem({ name, path, depth }: { name: string; path: string; depth: number }): JSX.Element {
  ...
  return (
    <>
      ...
      {open &&
        (children ?? []).map((child) => (
          <TreeItem key={child.path} name={child.name} path={child.path} depth={depth + 1} />
        ))}
    </>
  );
}
      ...
      <Section title="此电脑">
        {volumes.map((volume) => (
          <div key={volume.path}>
            ...
            {volume.path === '/' && <TreeItem name="用户" path="/Users" depth={1} />}
          </div>
        ))}
      </Section>
    </aside>
